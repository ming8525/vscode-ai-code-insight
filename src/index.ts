...
async function getMaxLines(): Promise<number> {
	const config = vscode.workspace.getConfiguration('aiCodeReview')
	return config.get<number>('maxLines') || 200
}

async function getMaxReviews(): Promise<number> {
	const config = vscode.workspace.getConfiguration('aiCodeReview')
	return config.get<number>('maxReviews') || 10
}
...
function createPrompt(fileName: string, code: string, maxLines: number = 100, maxReviews: number = 10) {
